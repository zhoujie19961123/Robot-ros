...
if nargval==1,
 ...
 for v=1:length(contr(:,1)),
  if contr(v,4)==0.5,
   ...
   if contr(v,1)==3, z=[z;-1]; end
   z=[z;ones(contr(v,2),1)];
   if contr(v,2)==3, p=[p;-1]; end
   if contr(v,1)>0, nc=nc+1; end
   if contr(v,2)>0, dc=dc+1; end
...
%   end
 ...
 if t1>0, z=[z;zeros(1,t1)];
 elseif t1<0, p=[p;zeros(1,-t1)]; end
