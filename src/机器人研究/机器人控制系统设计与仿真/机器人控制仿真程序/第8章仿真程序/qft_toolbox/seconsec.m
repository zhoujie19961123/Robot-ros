...
rs=real(s);  is=imag(s);
A=[[1;0],[rs';is']];
if length(T), A=[[1,1/(1-wm),1/(1-wm)/(1-wm1)];A];
...
te = [1, (1-2*length(T))*wm];  te1 = [1, (1-2*length(T))*wm1];
if length(T),
...
if length(T),
 ...
 nn1=(1-2*cos(b*T)*exp(-a*T)+exp(-2*a*T))*[1,0];
...
if delph>0,                               % pole or zero type?
 temp=dd1; dd1=nn1; nn1=temp;
end

% assign "best" solution
nn=[]; dd=[]; in=[];
if length(T),                             % find rhp zeros
 in=find(abs(zero2) >=1);
else
