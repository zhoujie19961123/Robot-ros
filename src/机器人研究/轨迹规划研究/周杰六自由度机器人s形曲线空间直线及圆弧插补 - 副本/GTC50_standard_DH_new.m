...
%利用SpaceLine在笛卡尔空间规划轨迹。
	...
	Qs=RtoQuaternions(T_S);%起点S的单元四元数Qs
    Qd=RtoQuaternions(T_D);%终点的单元四元数Qd
    v0 = 0;%返回一个与q0相同的矩阵，其所有元素为零
	v1 = 0;
    vmax =30;
    amax =20;
    jmax =10;
    %空间单一直线位置插补与单元四元数姿态插补 + S型加减速曲线归一化处理
    %起点S的单元四元数Qs，终点的单元四元数Qd
    t=0.01;%插值周期
    [x,y,z,qk,N]=SpaceLine_Q(S, D, Qs, Qd, v0, v1, vmax, amax, jmax, t);% 直线插补，得到插值点（不包括起点和终点）
    plot3(x, y, z);
    hold on
...
T = {1, N};
for i = 1: N
    R = QuaternionstoR(qk(i,:));%将四元数转为位姿矩阵
    R(1, 1:3);%R第一行
    R(2, 1:3);%R第二行
    R(3, 1:3);%R第三行
    T{i} = [R(1, 1:3), x(i);
...
%% 自己编写的空间圆弧插补程序
    ...
    T_D = Six_Link.fkine(Final_Theta);    %末端点末端执行器位姿
	Qs=RtoQuaternions(T_S);%起点S的单元四元数Qs
    Qd=RtoQuaternions(T_D);%终点的单元四元数Qd
    S = T_S(1: 3, 4); % 起点对应的位置坐标
    M = T_M(1: 3, 4);%中间点
    D = T_D(1: 3, 4); % 终点对应的位置坐标
    %空间单一圆弧位置插补与单元四元数姿态插补 + S型加减速曲线归一化处理
    v0 = 0;%返回一个与q0相同的矩阵，其所有元素为零
	v1 = 0;
    vmax =30;
    amax =20;
    jmax =10;
    t=0.01;%插值周期
    [x,y,z,qk,N] = SpaceCircle_Q(S', M', D', Qs, Qd, v0, v1, vmax, amax, jmax, t);%圆弧插补
    plot3(x, y, z)
    hold on
    th1(1) = First_Theta(1); th2(1) = First_Theta(2); th3(1) = First_Theta(3);
    th4(1) = First_Theta(4); th5(1) = First_Theta(5); th6(1) = First_Theta(6); 
